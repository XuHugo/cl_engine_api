...
mod tests {
    ...
    #[test]
    fn test_roundtrip() {
        let jwt_secrt = "f95c94e1843a5532d7f6f77713e58b2a76f48fff4862e9d7e126d7de3d50b67f";
        println!("jwt_secrt: {:?}", jwt_secrt.as_bytes());
        let jwt = hex::decode(jwt_secrt).unwrap();
        println!("jwt: {:?}", jwt);
        let auth = Auth::new(
            //JwtKey::from_slice(&DEFAULT_JWT_SECRET).unwrap(),
            JwtKey::from_slice(&jwt).unwrap(),
            Some("42".into()),
        ...
        assert_eq!(
            Auth::validate_token(&token, &JwtKey::from_slice(&jwt).unwrap())
                .unwrap()
                .claims,
            claims
        );
